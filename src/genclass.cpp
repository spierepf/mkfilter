...
static void readcmdline(const char * const argv[])
{
	...
	until (argv[ap] == NULL)
	{
		...
		else if (seq(s, "-n")) strcpy(name, argv[ap++]);
		else usage();
	}
...
static void pr_array_init(int count)
{
	printf("{0");
	for(int i = 1; i < count; ++i) printf(",0");
	printf("}");
}
...
static void comp_iir()
{
	...
	printf("{\n");
	printf("private:\n");
	printf("    static const int NZEROS = %d;\n", nzeros);
	printf("    static const int NPOLES = %d;\n", npoles);
	printf("    static constexpr float GAIN = %15.9e;\n", pbgain);
	printf("\n");
	printf("    float xv[NZEROS+1] = "); pr_array_init(nzeros+1); printf(";\n");
	printf("    float yv[NPOLES+1] = "); pr_array_init(npoles+1); printf(";\n");
	printf("\n");
	printf("public:\n");
	printf("    float operator()(float sample)\n");
	printf("    {\n");
	printf("        "); pr_shiftdown("xv", nzeros); putchar('\n');
	printf("        xv[%d] = sample / GAIN;\n", nzeros);
